 ...
 */
const core = async (isLast, reducer, type, scale, imgTop, noShadow = false, imgSide) => {
  if (DEBUG) {
  ...
  let scaleSide = scaleF(await loadImage(imgSide ? imgSide : imgTop), scale);
  let sideR = noShadow ? scaleSide : shadow(scaleSide, 2);
  let sideL = noShadow ? scaleSide : shadow(scaleSide, 1);
...
const checker = async ({type, renderTransparent, renderSides, noShadow, scale, reducer}) => {
  ...
  for (let i = 0; i < blocks.length; i += 1) {
    ...
    core(i === blocks.length-1, reducer, type, scale, currImage, noShadow);
  }
}
...
const itemToBase = async (file) => {
  ...
  const ctx = canvas.getContext('2d');
  let image = await loadImage(`${Items}${file}`);
...
const convertItems = async () => {
  ...
  for (let i = 0; i < items.length; i += 1) {
    let icon = await itemToBase(items[i]);
